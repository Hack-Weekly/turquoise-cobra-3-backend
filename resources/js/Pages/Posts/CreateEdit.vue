...
<template>
  ...
  <AuthenticatedLayout>
      ...
      <div class="w-3/12">
        ...
        <div class="content-card">
          ...
          <select v-model="tag.id" class="select select-primary w-full">
            ...
            <option v-for="tag in tags" :key="`tag-option${tag.id}`" :value="tag.id">{{tag.tag}}</option>
          </select>
    ...
    <button v-if="mode=='create'" class="btn btn-primary w-full" @click="savePost">Save Post</button>
